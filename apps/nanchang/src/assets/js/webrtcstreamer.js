let WebRtcStreamer = (function () {
  ...
  let WebRtcStreamer = function WebRtcStreamer(videoElement, srvurl) {
    ...
    /*this.iceServers = [
      {
        urls: ['stun:10.11.3.123:3478', 'turn:10.11.3.123:3478'],
        username: 'root',
        credential: '123',
      },
    ];*/
    ...
    try {
      ...
      this.pc.createOffer(this.mediaConstraints).then(
        (sessionDescription) => {
          ...
          console.log(
            `video codecs:${Array.from(
              new Set(RTCRtpReceiver.getCapabilities('video')?.codecs?.map((codec) => codec.mimeType))
            )}`
          );
          console.log(
            `audio codecs:${Array.from(
              new Set(RTCRtpReceiver.getCapabilities('audio')?.codecs?.map((codec) => codec.mimeType))
            )}`
          );*/
