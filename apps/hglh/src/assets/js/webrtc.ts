...
class Webrtc extends RtcEvent {
  ...
  constructor(
    ...
    this.serverUrl = import.meta.env.VITE_API_STREAM_URL;
    this.lastReport = {};
    this.timer = useIntervalFn(this.adjustResolutionBasedOnNetwork.bind(this), 1000);
    if (!import.meta.env.VITE_API_STREAM_URL) console.warn('媒体服务器地址未给！');
...
class webSocketVideo extends RtcEvent {
  ...
  constructor(videoEl: HTMLCanvasElement, params: string) {
    ...
    this.url = '';
    this.mseUrl = `${import.meta.env.VITE_API_AI_STREAM_URL}`;
