...
export const getAlarmListApi = (params: AlarmListParams) => {
  return http.get<ResPage<AlarmListRows>>(`${PORT_INSPECT}/abnormalItem/list`, params);
};
...
export const getAlarmDetailApi = (params: { id: string }) => {
  return http.get<AlarmListRows>(`${PORT_INSPECT}/abnormalItem/detail`, params);
};
export const getAlarmCheckApi = (params: {
  id: string;
  erasingExplain: string;
  erasingDateStart: string;
  erasingDateEnd: string;
}) => {
...
export const exportExcel = (params: any) => {
  return http.get<Blob>(`${PORT_INSPECT}/abnormalItem/export`, params, { responseType: 'blob', timeout: 0 });
};
