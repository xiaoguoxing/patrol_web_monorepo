...
async function getUrl() {
  let res = ``;
  res = `rtsp://${await decryptPassword(cameraData.value.cameraAccount!)}:${await decryptPassword(
    cameraData.value.cameraPassword!
  )}@${await decryptPassword(cameraData.value.cameraHost!)}:554/Streaming/Channels/${
    loginSendData.value?.channelNum || 1
  }01?transportmode=unicast`;
