...
function handleClick(e: MouseEvent) {
  ...
  setDrawArc(offsetX, offsetY, props.arcType, false);
  emit('addArc', {
    type: props.arcType,
    x: offsetX / offset.value,
    y: offsetY / offset.value,
    isConfirm: false,
    value: '',
  });
}
...
useEventListener(event, 'click', handleClick);
...
function exportImg() {
  ...
  // ctx.drawImage(arc.value!, 0, 0, bgImage.width, bgImage.height);
  props.arcList.forEach((i) => {
    ctx.beginPath();
    ctx.arc(i.x, i.y, confirmIcon, 0, Math.PI * 2, true);
    ...
    switch (i.type) {
      ...
      case 'arcCenter':
        ctx.lineWidth = 5;
        ctx.beginPath();
        ctx.arc(i.x, i.y, 5, 0, Math.PI * 2, true);
        ctx.fillStyle = props.arcColor;
        ctx.strokeStyle = '#fff';
        ctx.stroke();
        ctx.fill();
