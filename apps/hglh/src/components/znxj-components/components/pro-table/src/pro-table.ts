...
import type { BreakPoint, Responsive } from '@/components/znxj-components/components/grid';
...
export type SearchProps<T = any> = {
  ...
  render?: (scope: { row: T }, params: { [p: string]: any }) => any; // 自定义表格搜索项（tsx语法）
  isPrependInput?: boolean; // 是否显示表单前置select
  PrependOption?: SearchProps & { PrependProp: string; enum?: EnumProps[] | ((params?: any) => Promise<any>) };
} & Partial<Record<BreakPoint, Responsive>>;

export interface ColumnProps<T = any>
  extends Partial<Omit<TableColumnCtx<T>, 'children' | 'renderHeader' | 'renderCell' | 'filters'>> {
  tag?: boolean; // 是否是标签展示
  isShowInputLabel?: boolean; // 是否显示表单label
