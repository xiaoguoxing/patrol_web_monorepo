...
import {
  ...
  stethoscopeDetail,
  Row,
} from '@/api/modules/optCenter/deviceManage/auscultation';
...
const getTableList = (params: any) => {
  ...
  return stethoscopeList(newParams);
...
async function getAusculId(id: string) {
  let { data } = await stethoscopeDetail({ id });
  return {
    ...data,
    stethoscopePassword: await decryptPassword(data.stethoscopePassword),
    stethoscopeHost: await decryptPassword(data.stethoscopeHost),
    stethoscopeAccount: await decryptPassword(data.stethoscopeAccount),
  };
}
// 批量删除表格数据
