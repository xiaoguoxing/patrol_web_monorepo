      ...
      >
        ...
        <template #tableHeader="scope">
          <el-button
            ...
            >{{ $t('buttonName.add') }}{{ $t('device.sensor') }}</el-button
          >
          <el-button
            v-auth="'batchDelete'"
            icon="Delete"
...
const openForm = async (title: string, rowData: {} = {}) => {
  ...
  if (title === t('buttonName.add')) {
    ...
    if (nodeData.areaType == 2) {
      ...
      ElMessage.warning(t('camera.tip10'));
    }
    /* } else {
    ...
    let params = {
      ...
      api: title == t('buttonName.edit') ? sensorUpdate : '',
      getTableList: proTable.value.getTableList,
    };
    formDialogRef.value.acceptParams(params);
  }
...
const batchDelete = async (id: string[]) => {
  await useHandleData(sensorDelete, { ids: id.join() }, t('camera.tip11'));
  proTable.value.clearSelection();
  proTable.value.getTableList();
};
//删除表格数据
const deleteData = async (row: any) => {
  await useHandleData(sensorDelete, { ids: row.id }, t('camera.tip11'));
  proTable.value.getTableList();
};
</script>
