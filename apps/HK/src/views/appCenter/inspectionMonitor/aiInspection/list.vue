  ...
  <div class="flex-1 two-col-page">
    <div class="two-col-page-lf flex-column">
      <div class="mb20 flx-justify-between">
        <span class="list-title">{{ $t('aiInspection.listTitle') }}</span>
      </div>
    ...
    >
      <div class="targetitem-name">
        {{ targetTask.inspectionTaskName }}<span class="txt-secondary">></span>{{ targetItem.areaName
...
import {
  ...
  getStatisticsApi,
  webSocketUrl,
} from '@/api/modules/appCenter/inspectionMonitor/aiInspection';
...
//信号列表数据
const dataSource = ref<AITask.ResList[]>([]);
...
const getTotalList = (res: { data: any[] }) => {
  let { data } = res;
  dataSource.value = data;
  if (data.length > 0) {
    activeTask.value = dataSource.value.find((task) => task.id == activeTask.value.id) || ({} as AITask.ResList);
    if (!activeTask.value.taskItemMapList) {
      activeTask.value = dataSource.value[0];
    }
    getActiveItem();
  } else {
    activeItem.value = {};
    activeTask.value = {} as AITask.ResList;
  }
};
const getActiveItem = () => {
  //集中所有巡检项
  let taskItems = activeTask.value.taskItemMapList
    .map((obj) => {
