<template>
  <el-popover placement="bottom" popper-class="user-header-nav-info" :width="160" trigger="hover">
    ...
    <div class="user-info__content">
      <div class="user-info__item" @click="downloadPlugin()">
        <!-- openDialog('infoRef') -->
        <el-icon class="icon"><Notification /></el-icon>
        <div class="text">下载视频插件</div>
      </div>
...
import { downPlugin } from '@/api/modules/common';
...
async function downloadPlugin() {
  let [res] = await getDict('video_plugin');
  window.open(res.label, '_target');
  // downPlugin();
}
</script>
<style lang="scss">
