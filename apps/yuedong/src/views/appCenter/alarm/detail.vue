...
let formDataConfig = ref<FormConfig[]>([
  ...
  { label: '识别结果', value: '80℃', prop: 'recognitionResult', type: 'text' },
  {
    label: '告警规则',
    value: '温度≥65℃',
    prop: 'alarmRules',
...
async function openDialog() {
  try {
    let { value: abc } = await ElMessageBox.prompt('是否消警？', '温馨提示', {
    ...
    await getAlarmCheckApi({ id: rows.value.id!, erasingExplain: abc ?? '' });
    await getDetail();
  } catch (e) {}
}
...
<template>
  <div class="alarm-add">
    <div class="alarm-left">
      ...
      <div class="alarm-left-content">
        ...
        <videoControls
          ...
          :start-time="Math.floor(new Date(rows.playbackStartTime as string).getTime() / 1000)"
          :end-time="Math.floor(new Date(rows.playbackEndTime as string).getTime() / 1000)"
          :play-type="5"
          :playMode="1"
          :buttonType="4"
          :show-controls="false"
          :camera-id="rows.cameraId!"
          :recordLocation="rows.recordLocation!"
          :business-id="rows.id"
        ></videoControls>
      </div>
    </div>
    <div class="alarm-right">
      ...
      <div class="alarm-right-content" :class="{ tab2: tabValue === '2' }">
      ...
      <HKcloud v-if="tabValue === '2'" :cameraId="rows.cameraId" :abc="rows.cameraId" class="hkCloud" />
    </div>
