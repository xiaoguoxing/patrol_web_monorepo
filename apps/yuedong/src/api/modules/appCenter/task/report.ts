...
export const getReportListApi = (params: ReportListParams, isLoading: boolean = true) => {
  return http.get<ReportListRows>(`${PORT_INSPECT}/task/report/list`, params, {
    headers: { noLoading: isLoading },
    timeout: 0,
  });
};
export const getReportDetailApi = (params: { id: string }) => {
  return http.get<ReportListRows>(`${PORT_INSPECT}/task/report/detail`, params);
};
export const getReportTaskDetailApi = (params: { taskId: string }) => {
  return http.get<ReportListRows>(`${PORT_INSPECT}/task/report/detail/taskId`, params);
};
export const getReportExportApi = (id: string) => {
  return http.get<Blob>(`${PORT_INSPECT}/task/report/export`, { id }, { responseType: 'blob', timeout: 0 });
};
export const getReportCreateApi = (params: ReportListParams) => {
  return http.get<ReportListRows>(`${PORT_INSPECT}/task/report/create`, params);
};
export const getReportCheckApi = (params: any) => {
  return http.post<ReportListRows>(`${PORT_INSPECT}/task/report/check`, params, {
