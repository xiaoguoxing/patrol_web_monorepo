...
export const useTheme = () => {
  ...
  const changePrimary = (val: string) => {
    ...
    for (let i = 1; i < 100; i++) {
      document.documentElement.style.setProperty(
        `--kr-color-primary-dark-${i}`,
        `${new TinyColor(themeConfig.value.primary).shade(i).toString()}`
      );
    }
    //主色的辅色：高亮色
    document.documentElement.style.setProperty(
      `--kr-color-primary-brighter`,
      `${new TinyColor(themeConfig.value.primary).lighten(12).saturate(13).toString()}`
    );
    //主色的浅色： 如侧边栏选中色
    document.documentElement.style.setProperty(
      `--kr-color-primary-lighter`,
      `${new TinyColor(themeConfig.value.primary).lighten(56).saturate(1).toString()}`
    );
  };
